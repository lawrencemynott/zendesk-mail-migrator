module ZendeskMailMigrator
  ...
  class Deliverer
    ...
    def build_user_lookup_request(search_string)
      escaped_path = URI.escape("/users.#{settings[:extension]}?query=#{search_string}")
    ...
    def create_default_post_request(ticket, path)
      ...
      request.basic_auth(settings[:username], settings[:password])
      request["Content-Type"] = "application/#{settings[:extension]}"
    ...
    def build_new_user_post_request(ticket)
      create_default_post_request(ticket, "/tickets.#{settings[:extension]}") do |request, ticket|
    ...
    def build_end_user_post_request(ticket)
      if settings[:x_on_behalf_of?]
        request = create_default_post_request(ticket, "/requests.#{settings[:extension]}") do |request, ticket|
          set_x_on_behalf_of(request, ticket)
        end
      else
        request = create_default_post_request(ticket, "/tickets.#{settings[:extension]}") do |request, ticket|
          set_requester_email(ticket)
        end
      end
      request
    ...
    def build_manual_post_request(ticket)
      create_default_post_request(ticket, "#{settings[:path]}.#{settings[:extension]}") do |request, ticket|
        if settings[:x_on_behalf_of?]
          set_x_on_behalf_of(request, ticket)
        else
          set_requester_email(ticket)
